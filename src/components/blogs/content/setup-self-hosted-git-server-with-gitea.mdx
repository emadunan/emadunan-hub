...
You can add SSH keys to your Gitea user account from Profile → SSH Keys

## 7. Backup Strategy

It’s crucial to back up both Gitea’s application data and Postgres database.
Here’s a simple script I used to take backup from my local machine (Dont run the script from server):
...
echo "[INFO] ✅ Full backup complete: $BACKUP_DIR/gitea-backup-$TIMESTAMP.tar.gz"
